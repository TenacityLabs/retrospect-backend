...
func (songStore *SongStore) DeleteSong(userID uint, capsuleID uint, songID uint) error {
	_, err := songStore.db.Exec("DELETE FROM songs WHERE id = ? AND userId = ? AND capsuleId = ?", songID, userID, capsuleID)
	return err
}
